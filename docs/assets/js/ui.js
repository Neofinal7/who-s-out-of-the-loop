...
let _homeIntroTimer=null;
function triggerHomeIntro(){
 const home=document.getElementById('s-home');
 if(!home) return;
 if(_homeIntroTimer){ clearTimeout(_homeIntroTimer); _homeIntroTimer=null; }
 home.classList.remove('home-intro');
 requestAnimationFrame(()=>{
  home.classList.add('home-intro');
  _homeIntroTimer=setTimeout(()=>home.classList.remove('home-intro'),520);
 });
}
function show(id,instant){
 ...
 if(id==='s-home'&&!instant) triggerHomeIntro();
}
...
function openRulesModal(){
 ...
 requestAnimationFrame(()=>renderRulesModal());
...
function goToPlayerSetup(mode){
 ...
 show('s-players');
 // The player grid is already kept in sync as players are added/removed.
 // Avoid rebuilding it on every entry so menu navigation stays snappy.
 updateCountLabel();
 homeMenuNavLock=false;
}
...
function rebuildPlayerGrid(keepValues){
 ...
 for(let i=0;i<count;i++)appendPlayerRow(frag,i,oldVals[i]||'');
 grid.appendChild(frag);
 updateCountLabel();
 const addBtn=document.getElementById('add-player-btn');
 if(addBtn)addBtn.style.display=count>=15?'none':'flex';
}
function refreshPlayerGridTexts(){
 const grid=document.getElementById('players-grid');
 if(!grid) return;
 const rows=[...grid.querySelectorAll('.player-row')];
 rows.forEach((r,i)=>{
  const ph=(T.placeholders&&T.placeholders[i])||T.defaultName(i);
  const slot=r.querySelector('.player-slot');
  const input=r.querySelector('.player-input');
  const hint=r.querySelector('.player-hint');
  const num=r.querySelector('.player-num');
  if(slot) slot.textContent=ph;
  if(input) input.placeholder=ph;
  if(num) num.textContent=AVATARS[i%AVATARS.length];
  if(hint) hint.textContent=T.dir==='rtl'?'غيّر الاسم هنا':'Tap to rename';
 });
 const addBtn=document.getElementById('add-player-btn');
 if(addBtn) addBtn.style.display=rows.length>=15?'none':'flex';
}
...
const _particleScenes=new WeakMap();
function _pickPartyColor(colors){
 return colors[Math.floor(Math.random()*colors.length)];
}
function _makePartyParticle(scene,opts,initial){
 const width=Math.max(scene.width,1);
 const height=Math.max(scene.height,1);
 const sizeMin=opts.sizeMin||2.3;
 const sizeMax=opts.sizeMax||8.2;
 const reduced=scene.reduced;
 const particle={
  x:Math.random()*width,
  y:initial?Math.random()*height:(-12-Math.random()*height*0.25),
  radius:((reduced?sizeMin+0.6:sizeMin)+Math.random()*(sizeMax-sizeMin))*0.5,
  vy:(reduced?12:18)+Math.random()*(reduced?18:28),
  vx:(Math.random()*2-1)*(reduced?3.5:7),
  wobbleAmp:(reduced?3:6)+Math.random()*(reduced?4:10),
  wobbleSpeed:0.7+Math.random()*1.6,
  angle:Math.random()*Math.PI*2,
  opacity:reduced?0.34+Math.random()*0.14:0.42+Math.random()*0.2,
  color:_pickPartyColor(scene.colors),
  accent:_pickPartyColor(scene.colors),
 };
 return particle;
}
function _resetPartyParticle(scene,particle,opts){
 const width=Math.max(scene.width,1);
 const sizeMin=opts.sizeMin||2.3;
 const sizeMax=opts.sizeMax||8.2;
 const reduced=scene.reduced;
 particle.x=Math.random()*width;
 particle.y=-12-Math.random()*scene.height*0.18;
 particle.radius=((reduced?sizeMin+0.6:sizeMin)+Math.random()*(sizeMax-sizeMin))*0.5;
 particle.vy=(reduced?12:18)+Math.random()*(reduced?18:28);
 particle.vx=(Math.random()*2-1)*(reduced?3.5:7);
 particle.wobbleAmp=(reduced?3:6)+Math.random()*(reduced?4:10);
 particle.wobbleSpeed=0.7+Math.random()*1.6;
 particle.angle=Math.random()*Math.PI*2;
 particle.opacity=reduced?0.34+Math.random()*0.14:0.42+Math.random()*0.2;
 particle.color=_pickPartyColor(scene.colors);
 particle.accent=_pickPartyColor(scene.colors);
}
function _resizePartyScene(scene){
 ...
 scene.width=width;
 scene.height=height;
 scene.canvas.width=Math.max(1,Math.round(width*dpr));
 scene.canvas.height=Math.max(1,Math.round(height*dpr));
 scene.canvas.style.width=`${width}px`;
 scene.canvas.style.height=`${height}px`;
 scene.ctx.setTransform(dpr,0,0,dpr,0,0);
}
function _drawPartyParticle(ctx,p){
 const x=p.x+Math.sin(p.angle)*p.wobbleAmp;
 const y=p.y;
 const r=p.radius;
 ...
 ctx.fill();
 ctx.globalAlpha=Math.min(1,p.opacity*0.62);
 ctx.fillStyle='rgba(255,255,255,.9)';
 ctx.beginPath();
 ctx.arc(x-r*0.32,y-r*0.34,Math.max(0.8,r*0.34),0,Math.PI*2);
 ctx.fill();
 ctx.globalAlpha=Math.min(1,p.opacity*0.48);
 ctx.fillStyle=p.accent;
 ctx.beginPath();
 ctx.arc(x+r*0.16,y+r*0.18,Math.max(0.7,r*0.24),0,Math.PI*2);
 ctx.fill();
}
function _animatePartyScene(scene,ts){
 if(!scene.layer.isConnected){
  if(scene.onResize) window.removeEventListener('resize',scene.onResize);
  _particleScenes.delete(scene.layer);
  return;
 }
 if(!scene.lastTs) scene.lastTs=ts;
 const dt=Math.min(32,ts-scene.lastTs);
 scene.lastTs=ts;
 const ctx=scene.ctx;
 ctx.clearRect(0,0,scene.width,scene.height);
 if(!document.hidden&&scene.width>0&&scene.height>0){
  for(let i=0;i<scene.particles.length;i++){
   const p=scene.particles[i];
   p.angle+=p.wobbleSpeed*(dt/1000);
   p.x+=p.vx*(dt/1000);
   p.y+=p.vy*(dt/1000);
   if(p.x<-16) p.x=scene.width+16;
   else if(p.x>scene.width+16) p.x=-16;
   if(p.y>scene.height+20){
    _resetPartyParticle(scene,p,scene.opts);
   }
   _drawPartyParticle(ctx,p);
  }
 }
 scene.raf=requestAnimationFrame(nextTs=>_animatePartyScene(scene,nextTs));
}
function buildPartyParticles(layer,opts={}){
 if(!layer||_particleScenes.has(layer)) return;
 ...
 _resizePartyScene(scene);
 const count=opts.count||(reduced?28:46);
 for(let i=0;i<count;i++) scene.particles.push(_makePartyParticle(scene,opts,true));
 scene.onResize=()=>_resizePartyScene(scene);
 window.addEventListener('resize',scene.onResize,{passive:true});
 _particleScenes.set(layer,scene);
 scene.raf=requestAnimationFrame(ts=>_animatePartyScene(scene,ts));
}
