...
/**
 * =========================================================
 * 🛠️ AUTO-FIX PATCH FOR GAME BUGS
 * =========================================================
 * Instructions: Paste this entire block at the VERY BOTTOM 
 * of your inline_script_1 file. It fixes the 3 issues automatically!
 */

(function applyGameFixes() {
    console.log("🛠️ Applying Game Fixes (Menu Lag, Timers, and Secret Twist)...");

    // ---------------------------------------------------------
    // FIX 1: Smooth out the Start Menu Button Lag
    // ---------------------------------------------------------
    // We inject a style to force the browser to use hardware acceleration for the buttons
    ...
    // We intercept the screen switching to give the browser time to breathe
    const origShowScreen = window.showScreen;
    if (typeof origShowScreen === 'function') {
        window.showScreen = function(screenId) {
            // Double requestAnimationFrame forces the layout to load BEFORE animations start
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    origShowScreen(screenId);
                    
                    // If returning to a menu, reset button animations smoothly
                    if (screenId.includes('home') || screenId.includes('menu') || screenId === 'screen-start') {
                        const screen = document.getElementById(screenId);
                        if (screen) {
                            const btns = screen.querySelectorAll('.btn, .menu-btn');
                            btns.forEach(btn => {
                                btn.style.animation = 'none'; // Stop current laggy animation
                                void btn.offsetWidth; // Force the browser to register the stop
                                btn.style.animation = ''; // Start it cleanly
                            });
                        }
                    }
                });
    ...
    // ---------------------------------------------------------
    // FIX 2: Stop the Timer Twist from running in the background
    // ---------------------------------------------------------
    window.activeGameTimers = [];
    const origSetInterval = window.setInterval;
    
    // Every time the game creates a countdown timer (usually 1000ms/1 second), we track it
    window.setInterval = function(fn, delay, ...args) {
        const id = origSetInterval(fn, delay, ...args);
        if (delay === 1000) window.activeGameTimers.push(id);
        return id;
    };

    // We run a silent check in the background. If the game screen is closed/hidden, kill the timers!
    setInterval(() => {
        const gameScreen = document.getElementById('screen-game') || document.querySelector('.game-active');
        
        // If the game screen doesn't exist or is currently hidden
        if (gameScreen && (gameScreen.style.display === 'none' || gameScreen.classList.contains('hide'))) {
            // Destroy all active countdowns so they don't break the next round
            window.activeGameTimers.forEach(id => clearInterval(id));
            window.activeGameTimers = [];
        }
    }, 1500);

    // ---------------------------------------------------------
    // FIX 3: Hide the player's identity in the Secret Info Twist
    // ---------------------------------------------------------
    // We watch the screen for any text that pops up
    const twistObserver = new MutationObserver((mutations) => {
        mutations.forEach(mutation => {
            ...
            if (target.nodeType === Node.ELEMENT_NODE || target.nodeType === Node.TEXT_NODE) {
                ...
                // If we already fixed this text, skip it
                if (!el || el.dataset.patchedSecret) return;
                
                const text = el.innerText || el.textContent;
                if (!text) return;

                // Check if the game is trying to announce the secret info twist
                const isArabic = text.includes('معلومة سرية') || text.includes('يعرف شيئاً');
                const isEnglish = text.toLowerCase().includes('secret info') || text.toLowerCase().includes('secret knowledge');

                if (isArabic || isEnglish) {
                    el.dataset.patchedSecret = "true"; // Mark as fixed so we don't loop endlessly
                    
                    // Replace the text that has their name with a generic anonymous message
                    const safeArabic = "🤫 أحد اللاعبين حصل على معلومة سرية! (هويته مخفية ليستخدمها في الوقت المناسب...)";
                    const safeEnglish = "🤫 A random player received secret info! (Their identity remains hidden...)";
                    
                    if (el.innerText) el.innerText = isArabic ? safeArabic : safeEnglish;
                    else el.textContent = isArabic ? safeArabic : safeEnglish;
                }
            }
        });
    });
    
    // Tell the observer to monitor the whole screen for changes
