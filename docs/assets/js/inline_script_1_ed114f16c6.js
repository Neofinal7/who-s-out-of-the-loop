...
function show(id,instant){
 ...
 const doShow=()=>{
  // Batch DOM writes: remove active from all, then add to target
  // requestAnimationFrame ensures this happens at the start of a frame, not mid-paint
...
function firePreWordTwist(){
 ...
 if(id==='insider_spy'){
  ...
  showTwistScreen(
   ...
   ar?'لاعب واحد سيحصل على معلومات سرية عن الجولة عند رؤية كلمته — بدون ما أحد يعرف مين.'
     :'One player will secretly receive full round intel on their private word screen — nobody else will know who.',
   `<div style="background:rgba(155,93,229,.1);border:2px solid var(--purple);border-radius:12px;padding:14px;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:var(--purple);">
    🕵️ ${ar?'المعلومة ستظهر سرياً لشخص واحد فقط عند دوره':'The intel appears privately for one player on their turn'}
   </div>`,
   ar
  );
  return;
...
function renderSpotlightQ(){
 ...
 if(timerWrap){
  if(isTimePressure&&!timerWrap.querySelector('.timer-ring')){
   ...
   // Reset display to 10 now that new DOM nodes exist
   const _n=timerWrap.querySelector('#timer-num');
   if(_n){_n.textContent=_timerSecsLeft;_n.classList.remove('timer-urgent');}
...
function startTimerCountdown(){
 ...
 // Always clear any existing interval first — prevents double-timers
 if(_timePressureInterval){ clearInterval(_timePressureInterval); _timePressureInterval=null; }
 let secs=_timerSecsLeft;
 // Re-query nodes each tick — never close over them so stale refs can't fire against replaced DOM
 _timePressureInterval=setInterval(()=>{
  // Stop immediately if questions are done or twist is no longer active
  if(currentQIdx>=questions.length||!(activeTwist&&activeTwist.id==='time_pressure')){
...
function goHome(){
 ...
 bar.classList.remove('visible'); document.body.classList.remove('has-scorebar');
 show('s-home');
 requestAnimationFrame(()=>{ setGenreMode('random'); setOutsiderMode('standard'); });
}
