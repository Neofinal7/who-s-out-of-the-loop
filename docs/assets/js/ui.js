...
function goToRoundSetup(){
 ...
 // First round of a new match — show forced options screen
 if(!matchActive){
  matchActive=true;
  currentMatchRound=0;
  matchScores={};
 ...
 // Subsequent rounds go straight to setup
 show('s-setup');
...
function showOptionsScreen(){
 ...
 const tt=document.getElementById('twists-toggle');if(tt)tt.classList.toggle('on',twistsOn);
 const gt=document.getElementById('gq-toggle');if(gt)gt.classList.toggle('on',gameQOn);
 // Sync toggles already done above
 // Show/hide imposters section based on player count
 const imp=document.getElementById('opt-imposters-section');
 if(imp){
  const show5=(players.length>=5);
  imp.style.display=show5?'block':'none';
 }
 // Sync imposter buttons
 const ib1=document.getElementById('imposters-btn-1');
 const ib2=document.getElementById('imposters-btn-2');
 if(ib1) ib1.className='tgl'+(outsiderCount===1?' sel':'');
 if(ib2) ib2.className='tgl'+(outsiderCount===2?' sel':'');
 // Labels
 const s=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
 ...
 s('opt-imposters-sub',ar?'متاح لـ 5 لاعبين أو أكثر':'Available for 5+ players');
 if(ib1) ib1.textContent=ar?'خارج واحد':'1 Outsider';
 if(ib2) ib2.textContent=ar?'خارجان':'2 Outsiders';
 show('s-options');
}
...
function setOutsiderCount(n){
 ...
 const ib1=document.getElementById('imposters-btn-1');
 const ib2=document.getElementById('imposters-btn-2');
 if(ib1) ib1.className='tgl'+(n===1?' sel':'');
 if(ib2) ib2.className='tgl'+(n===2?' sel':'');
}

function confirmOptions(){
 playSound('start');haptic('heavy');
 // Safety: if player count is less than 5, force single outsider
...
// INIT
// ════════════════════════════════════════════════════════════════
// Defer heavy init — give splash 2 frames to paint before DOM work
setTimeout(()=>{
 rebuildPlayerGrid(false);
 applyTranslations();
},120);