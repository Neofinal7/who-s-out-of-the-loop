...
function firePreWordTwist(){
 ...
 if(!id){ activeTwist=null; showCover(); return; }
 activeTwist={id};
...
function showMatchOver(){
 ...
 const winnerName=winner?winner[0]:'?';
 const winnerPts=winner?winner[1]:0;
 const runnerUp=sorted[1];
 const runnerUpName=runnerUp?runnerUp[0]:'—';
 const runnerUpPts=runnerUp?runnerUp[1]:0;
 const leadMargin=runnerUp?Math.max(0,winnerPts-runnerUpPts):winnerPts;
 const roundsPlayed=matchRoundHistory.length;
 ...
 document.getElementById('match-winner-wrap').innerHTML=`
  <div class="match-winner-card" style="padding:22px 18px;">
   ...
   <div style="width:100%;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:8px;">
    <div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:10px 8px;">
     <div style="font-size:12px;color:var(--muted);margin-bottom:4px;">${T.roundLblShort||'Round'}</div>
     <div style="font-size:18px;font-weight:900;">${roundsPlayed}/${matchRoundCount}</div>
    </div>
    <div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:10px 8px;">
     <div style="font-size:12px;color:var(--muted);margin-bottom:4px;">${T.ptsLbl||'pts'} ${T.dir==='rtl'?'فرق':'gap'}</div>
     <div style="font-size:18px;font-weight:900;">+${leadMargin}</div>
    </div>
    <div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:10px 8px;">
     <div style="font-size:12px;color:var(--muted);margin-bottom:4px;">${T.dir==='rtl'?'الوصيف':'Runner-up'}</div>
     <div style="font-size:18px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${runnerUpName}</div>
    </div>
   </div>
  </div>`;
