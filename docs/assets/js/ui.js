...
const _MOTION_PREWARM_SEL='.btn,.tgl,.btn-add-player,.lang-btn,.menu-grid > div,.genre-card,.vote-option,.gq-choice,.punish-opt,.guess-opt';
...
function _setScreenRendered(el,isRendered){
 if(!el) return;
 if(isRendered){
  el.style.removeProperty('display');
  return;
 }
 // Hidden screens must fully leave layout/rendering, otherwise Arabic text keeps
 // paying shaping/layout costs even when the screen is not visible.
 el.style.setProperty('display','none','important');
}
_allScreens.forEach(el=>_setScreenRendered(el,el===_activeScreen));
function _prewarmAnimatedScope(root){
 if(!root||typeof root.querySelectorAll!=='function') return;
 const targets=root.matches&&root.matches(_MOTION_PREWARM_SEL)
  ? [root,...root.querySelectorAll(_MOTION_PREWARM_SEL)]
  : root.querySelectorAll(_MOTION_PREWARM_SEL);
 for(const el of targets){
  window.getComputedStyle(el).transform;
  try{ window.getComputedStyle(el,'::after').transform; }catch(_){}
 }
}
function show(id,instant){
 ...
 clearUiPress();
 if(_activeScreen && _activeScreen!==el){
  _activeScreen.classList.remove('active');
  _setScreenRendered(_activeScreen,false);
 }
 _setScreenRendered(el,true);
 _prewarmAnimatedScope(el);
 if(window.refreshWiggles) window.refreshWiggles(el);
 el.scrollTop=0;
 _activeScreen=el;
 el.classList.add('active');
}
function closeModal(id,e){if(e.target===document.getElementById(id))closeModalDirect(id);}
function closeModalDirect(id){const el=document.getElementById(id);if(el){el.classList.add('hidden');if(window.refreshWiggles) window.refreshWiggles(_activeScreen||document.body);}}
function openSettings(){getAudio();playSound('settings');const modal=document.getElementById('settings-modal');modal.classList.remove('hidden');_prewarmAnimatedScope(modal);if(window.refreshWiggles) window.refreshWiggles(modal);}
function openRulesModal(){
 ...
 _prewarmAnimatedScope(modal);
 if(window.refreshWiggles) window.refreshWiggles(modal);
...
function applyTranslations(){
 ...
 const ar=T.dir==='rtl';
 const appTitle=T.appTitle||(ar?'مين خارج اللعبة':"Who's Out of the Loop?");
 ...
 if(splash){
  ...
  if(sub) sub.textContent=T.splashSub||(ar?'يقلب اللعبة بين الأصدقاء — مين فهم كلمة مختلفة أولاً':'A party game of hidden words and bluffing');
 }
 ...
 if(install){
  ...
  if(t) t.textContent=T.installTitle||(ar?'حمّل التطبيق':'Install the app');
  if(sub) sub.textContent=T.installSub||(ar?'العب بدون انترنت':'Play offline anytime');
  if(btn) btn.textContent=T.installBtn||(ar?'تثبيت':'Install');
 }
 ...
 s('rules-close-btn','✕');
 // Image strip labels
 s('home-strip-lbl', T.homeStripLbl||(ar?'العب مع أصحابك ✨':'Play with your crew ✨'));
 s('players-strip-lbl', T.playersStripLbl||(ar?'مين معاك الليلة؟ 👥':"Who's playing tonight? 👥"));
 s('tally-strip-lbl', T.tallyStripLbl||(ar?'الجمهور اختار — والنتيجة؟':"The crowd voted — the verdict?"));
 s('reveal-strip-lbl', T.revealStripLbl||(ar?'اللحظة الحقيقية':'The Moment of Truth'));
 s('matchover-strip-lbl', T.matchoverStripLbl||(ar?'بطل اللعبة':'Match Champion'));
 s('cover-strip-lbl', T.coverStripLbl||(ar?'دورك — خذ التلفون':'Your turn — take the phone'));
 s('setup-strip-lbl', T.setupStripLbl||(ar?'جهّزوا الجولة':'Set up the round'));
...
function appendPlayerRow(grid,idx,val){
 ...
 const hint=T.renameHint||(T.dir==='rtl'?'غيّر الاسم هنا':'Tap to rename');
...
function updateCountLabel(){
 ...
 if(el){
  ...
  const hint=T.lineupHint||(T.dir==='rtl'?'رتبوا الأسماء قبل البداية':'Set your lineup before starting');
...
function syncTwistCountButtons(){
 ...
 if(note){
  ...
  note.textContent=twistsOn
   ...
   :(T.turnTwistsOn||(ar?'فعّل التحديات أولاً':'Turn twists on first'));
 }
...
function renderRulesModal(){
 const ar=T.dir==='rtl';
 const localizedRules=window.getRulesCopy?window.getRulesCopy(lang):null;
 const sections=localizedRules?localizedRules.sections:(ar?[
  {
   e:'🎯',
   t:'الهدف',
   items:[
 ...
 if(content){
  ...
  if(localizedRules){
   const quickBlock=content.firstElementChild?.firstElementChild;
   const quickChildren=quickBlock?quickBlock.querySelectorAll('div'):null;
   if(quickChildren&&quickChildren[0]) quickChildren[0].textContent=localizedRules.quickStartTitle;
   if(quickChildren&&quickChildren[1]) quickChildren[1].textContent=localizedRules.quickStartBody;
  }
 }
}
...
function _syncOutsiderButtons(){
 ...
 if(ib1){
  ...
  ib1.textContent=T.outsiderCountOne||(ar?'1 خارج':'1 Outsider');
 }
 if(ib2){
  if(enough){
   ib2.className='tgl'+(outsiderCount===2?' sel':'');
   ib2.textContent=T.outsiderCountTwo||(ar?'2 خارجين':'2 Outsiders');
  } else {
   ib2.className='tgl tgl-locked';
   ib2.textContent=T.outsiderCountTwoLocked||(ar?'2 خارجين 🔒':'2 Outsiders 🔒');
  }
 }
 if(note) note.style.display=enough?'none':'block';
 if(ib1&&T.outsiderCountOne) ib1.textContent=T.outsiderCountOne;
 if(ib2){
  if(enough&&T.outsiderCountTwo) ib2.textContent=T.outsiderCountTwo;
  if(!enough&&T.outsiderCountTwoLocked) ib2.textContent=T.outsiderCountTwoLocked;
 }
}
...
function showOptionsScreen(){
 ...
 _syncOutsiderButtons();
 const s=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
 s('opt-screen-badge',T.gameOptionsBadge||(ar?'🎛️ خيارات اللعبة':'🎛️ Game Options'));
 s('opt-screen-title',T.gameOptionsTitle||(ar?'خيارات اللعبة':'Game Options'));
 s('opt-screen-sub',T.gameOptionsSub||(ar?'اختر إعدادات اللعبة قبل ما تبدأوا':'Set your game options before starting'));
 s('btn-confirm-options',T.confirmContinue||(ar?'✅ تأكيد وكمّل':'✅ Confirm & Continue'));
 s('opt-imposters-lbl',T.optImpostersLbl||(ar?'🕵️ عدد الخارجين':'🕵️ Number of Outsiders'));
 s('btn-back-options',T.btnBack||(ar?'← رجوع':'← Back'));
...
function generateIcon(size){
 ...
 const ar=T.dir==='rtl';
 const appTitle=T.appTitle||(ar?'مين خارج اللعبة':"Who's Out of the Loop?");
const MANIFEST={name:appTitle,short_name:(T.appTitleShort||appTitle).slice(0,12),display:'standalone',
 orientation:'portrait',background_color:'#ff5e99',theme_color:'#ff5e99',lang:lang,dir:T.dir||'ltr',
