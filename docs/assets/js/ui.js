...
function buildPartyParticles(layer,opts={}){
 ...
 const frag=document.createDocumentFragment();
 const count=opts.count||(reduced?31:53);
 for(let i=0;i<count;i++){
  ...
  const duration=reduced?12+Math.random()*6:durationMin+Math.random()*(durationMax-durationMin);
  const color=colors[Math.floor(Math.random()*colors.length)];
  const accent=colors[(Math.floor(Math.random()*colors.length)+1)%colors.length];
  const spreadRoll=Math.random();
  const left=spreadRoll<0.2?Math.random()*20:
   spreadRoll<0.4?80+Math.random()*20:
   spreadRoll<0.6?20+Math.random()*60:
   Math.random()*100;
  const top=spreadRoll<0.2?Math.random()*30:
   spreadRoll<0.4?70+Math.random()*30:
   spreadRoll<0.6?10+Math.random()*80:
   Math.random()*100;
  particle.className='party-particle';
  particle.style.left=`${left}%`;
  particle.style.top=`${top}%`;
  particle.style.setProperty('--size',`${size.toFixed(1)}px`);
  particle.style.setProperty('--drift',`${Math.round((Math.random()*2-1)*(reduced?40:140))}px`);
  particle.style.setProperty('--travel',`${110+Math.round(Math.random()*(reduced?50:200))}vh`);
  particle.style.setProperty('--duration',`${duration.toFixed(1)}s`);
  particle.style.setProperty('--delay',`${-(Math.random()*(reduced?20:18)).toFixed(1)}s`);
  particle.style.setProperty('--opacity',(reduced?0.35+Math.random()*0.16:0.42+Math.random()*0.24).toFixed(2));
