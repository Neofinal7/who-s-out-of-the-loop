(function () {
  ...
  function isRenderable(entry) {
    ...
    if (!isInVisibleScope(entry.el)) return false;
    return true;
  }

  function setAnimationHint(target, active) {
    if (!target) return;
    if (active) {
  ...
  function finishEntry(entry, token) {
    ...
    setAnimationHint(entry.target, false);
  ...
  function unregister(el) {
    ...
    if (!entry) return;
    stopEntry(entry);
    if (entry.prebuiltAnim) {
      try {
        entry.prebuiltAnim.cancel();
      } catch (err) {}
      entry.prebuiltAnim.onfinish = null;
      entry.prebuiltAnim = null;
    }
    if (entry.target) entry.target.style.removeProperty('will-change');
  ...
  function startWiggle(entry) {
    ...
    setAnimationHint(target, true);

    if (entry.prebuiltAnim) {
      cancelAnimation(target);
      const anim = entry.prebuiltAnim;
      target._wiggleAnim = anim;
      anim.onfinish = function () { finishEntry(entry, token); };
  ...
  function register(el, keyframe, duration) {
    ...
    // Keep visible wiggle targets pre-promoted so the start of the animation
    // doesn't hitch when text-heavy buttons/cards begin moving.
    el.style.willChange='transform';
    if (
      CFG.useWebAnimations &&
      typeof KeyframeEffect === 'function' &&
      typeof Animation === 'function' &&
      document.timeline
    ) {
      try {
        const effect = new KeyframeEffect(el, entry.frames, {
          duration: Math.round(duration * entry.durationScale),
          easing: CFG.easing,
          fill: 'none',
        });
        const anim = new Animation(effect, document.timeline);
        anim.pause();
        entry.prebuiltAnim = anim;
      } catch (err) {}
    }
  ...
  function registerAll() {
    ...
    if (activeScreen) registerWithin(activeScreen);
    document.querySelectorAll('.modal-overlay:not(.hidden)').forEach(function (modal) {
      registerWithin(modal);
    });
  }

  function purgeHiddenEntries() {
    registry.forEach(function (entry, el) {
      if (!el.isConnected || !isInVisibleScope(el)) unregister(el);
    });
  }
  ...
  function queueScan(root) {
    ...
    if (root === document.body) {
      ...
      for (const queuedRoot of Array.from(queuedRoots)) {
        if (queuedRoot === root || queuedRoot.contains(root)) return;
        if (root.contains(queuedRoot)) queuedRoots.delete(queuedRoot);
      }
      queuedRoots.add(root);
    }
  ...
  function getScanRoot(node) {
    if (!node || node.nodeType !== 1) return null;
    if (typeof node.matches === 'function') {
      if (node.matches(HOME_CARD_SEL)) return node;
      if (node.matches(BTN_SEL) && !node.closest(HOME_CARD_SEL)) return node;
    }
    return node.closest('.screen.active, .modal-overlay:not(.hidden)') || null;
  }
  ...
  const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      ...
      mutation.addedNodes.forEach(function (node) {
        ...
        const root = getScanRoot(node);
        if (root) queueScan(root);
      });
    });
  });
  ...
  window.refreshWiggles = function (root) {
    purgeHiddenEntries();
    queueScan(root && root.nodeType === 1 ? root : document.body);
  };
