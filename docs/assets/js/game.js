...
function firePreWordTwistSilent(){
 ...
 if(id==='word_swap'){
  ...
  showTwistScreen(
   '🔄',
   ar?'تبديل كلمات':'Word Swap',
   ar?`${players[a]} و ${players[b]} رح يبدلوا الكلمات أول ما تبدأ الجولة.`
     :`${players[a]} and ${players[b]} will swap words when the round starts.`,
   `<div style="background:rgba(255,165,0,.1);border:2px solid var(--orange);border-radius:12px;padding:14px;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:var(--orange);">
    🔄 ${ar?'التبديل نفسه معلن، لكن الكلمات تبقى سرية لكل لاعب':'The swap is public, but each word stays secret to its player'}
   </div>`,
   ar
  );
  consumeEvent();
...
function fireRandomTwist(){
 ...
 if(id==='forced_vote'){
  ...
  desc=ar
   ...
   :`${players[pIdx]} — say the THIRD letter of your word out loud for everyone!`;
  ...
  desc=ar
   ...
   :`${players[pIdx]} — say how many letters your word has out loud!`;
  ...
  desc=ar
   ...
   :`${players[pIdx]} — give the group two clues about your word: one true and one fake.`;
...
// COVER + WORD SCREENS
// ════════════════════════════════════════════════════════════════
let _progDotsEl=null;
function updateDots(){
 if(!_progDotsEl||!_progDotsEl.isConnected) _progDotsEl=document.getElementById('prog-dots');
 const d=_progDotsEl;
 if(!d) return;
 // Only rebuild innerHTML if dot count changed; otherwise just toggle classes
 if(d.children.length!==players.length){
  ...
  for(let i=0;i<d.children.length;i++){
   const want=i<currentIdx?'dot done':i===currentIdx?'dot active':'dot';
   if(d.children[i].className!==want) d.children[i].className=want;
  }
 }
}
...
function showCover(){
 ...
 // Use cached refs — cover DOM never rebuilds, so cache persists for the whole match
 if(!_coverEls){
  _coverEls={
   avatar:document.getElementById('cover-avatar'),
   name:document.getElementById('cover-name'),
   name2:document.getElementById('cover-name2'),
   passText:document.getElementById('cover-pass-text'),
   eyesText:document.getElementById('cover-eyes-text'),
   warning:document.getElementById('cover-warning'),
  };
 }
 _coverEls.avatar.textContent=av;
 _coverEls.name.textContent=name;
 _coverEls.name2.textContent=name;
 _coverEls.passText.textContent=T.coverPassText;
 _coverEls.eyesText.textContent=T.coverEyesText;
 _coverEls.warning.textContent=T.coverWarning;
 show('s-cover');
}
...
// QUESTIONS PHASE
// ════════════════════════════════════════════════════════════════
// DOM cache: avoids repeated getElementById on every question advance and every player turn.
// Invalidated whenever the q-spot shell or cover screen is rebuilt.
let _qSpotCache=null;
let _coverEls=null;
...
function syncTimePressureDisplay(secs,resetArc){
 if(!_timerEls||!_timerEls.num||!_timerEls.num.isConnected){
  _timerEls={num:document.getElementById('timer-num'),arc:document.getElementById('timer-arc')};
 }
 const num=_timerEls.num;
 const arc=_timerEls.arc;
...
function renderSpotlightQ(){
 ...
 // Patch using cached refs — invalidate cache if shell was just rebuilt
 if(!_qSpotCache||!_qSpotCache.card||!_qSpotCache.card.isConnected){
  _qSpotCache={
   card:document.getElementById('q-spot-card'),
   timerWrap:document.getElementById('q-timer-wrap'),
   num:document.getElementById('q-num-el'),
   asker:document.getElementById('q-asker-el'),
   name:document.getElementById('q-name-el'),
   targetAv:document.getElementById('q-target-av-el'),
   targetName:document.getElementById('q-target-name-el'),
   dots:document.getElementById('q-dots-el'),
  };
 }
 const card=_qSpotCache.card;
 if(card){
 ...
 // Patch timer
 const timerWrap=_qSpotCache.timerWrap;
 if(timerWrap){
 ...
 // Patch text/emoji nodes directly — use cached refs, zero getElementById calls
 const numEl=_qSpotCache.num;
 if(numEl){
  numEl.textContent = isBonus 
    ? `★ ${T.bonusLabel} / ${questions.length}` 
    : `#${currentQIdx+1} / ${questions.length}`;
  numEl.style.color = cc; 
  numEl.style.background = cc + '18';
}
 const askerEl=_qSpotCache.asker;
 if(askerEl) askerEl.textContent=AVATARS[q.asker%AVATARS.length];
 const nameEl=_qSpotCache.name;
 if(nameEl) nameEl.textContent=players[q.asker]||'?';
 const targetAvEl=_qSpotCache.targetAv;
 if(targetAvEl) targetAvEl.textContent=AVATARS[q.target%AVATARS.length];
 const targetNameEl=_qSpotCache.targetName;
...
let _qRemainEl=null;
function updateQRemain(n){ if(!_qRemainEl) _qRemainEl=document.getElementById('q-remain'); if(_qRemainEl) _qRemainEl.textContent=n; }
 ...
 phasesDiv.innerHTML=`
  <div class="vote-shell">
   ...
   <div class="vote-notes">
    ...
    ${hasTrustLock?`<div class="vote-note vote-note-lock">🔒 ${ar?`لا يمكن التصويت ضد ${players[trustLockedTarget]}`:`Cannot vote against ${players[trustLockedTarget]}`}</div>`:''}
   </div>
   <div class="vote-panel">
    <div class="vote-panel-head">
     <div>
      <div class="vote-panel-title">${ar?'اختار التصويت':'Choose a vote'}</div>
      <div class="vote-panel-sub">${ar?'اضغط على اسم اللاعب اللي تبغى تصوّت له':'Tap the player you want to vote for'}</div>
     </div>
...
function renderPunishment(groupWon, counts){
 ...
 wrap.innerHTML=`
  <div class="punish-wrap" style="text-align:center;gap:14px;">
   ...
   <div style="font-size:13px;color:var(--muted);margin-bottom:4px;">${T.dir==='rtl'?'العجلة تختار التحدي النهائي':'The wheel chooses the final challenge'}</div>
 ...
 function step(){
  if(tick>=totalTicks){
   ...
   setTimeout(()=>{
    ...
    if(chosen==='guess'&&canGuess){area.innerHTML=`<div class="punish-result" style="background:linear-gradient(135deg,rgba(61,220,90,.12),rgba(61,220,90,.06));border-color:rgba(61,220,90,.35);color:var(--green);">${labelPill}<div>${T.dir==='rtl'?'فرصة ثانية!':'Second chance!'}</div></div>`;showGuessChallenge(area);}
