...
// TOPIC / STORY PICKERS
// ════════════════════════════════════════════════════════════════
const RANDOM_SIMILAR_TOPIC_BASE_CHANCE=0.18;
const RANDOM_SIMILAR_TOPIC_COOLDOWN_CHANCE=0.05;
const RANDOM_SIMILAR_TOPIC_KEYS=new Set([
 'food::pizza::pie',
 'food::burger::hot dog',
 'food::sushi::ramen',
 'food::chicken wrap::burger',
 'food::fried egg::omelette',
 'food::broth::stew',
 'food::cheese::butter',
 'sweets::tiramisu::profiteroles',
 'sweets::cupcake::muffin',
 'sweets::chocolate::caramel',
 'sweets::ice cream::gelato',
 'sweets::waffle::pancake',
 'sweets::brownie::mini donuts',
 'sweets::croissant::cookie',
 'sweets::creme brulee::pudding',
 'drinks::cappuccino::latte',
 'drinks::bubble tea::milk tea',
 ...
 'animals::wolf::arctic fox',
 'animals::eagle::falcon',
 'animals::flamingo::peacock',
 'animals::crocodile::lizard',
 'animals::lion::tiger',
 'animals::elephant::hippopotamus',
 'animals::penguin::seal',
 'animals::fox::wolf',
 'animals::dolphin::whale',
 'animals::parrot::toucan',
 'animals::shark::sawfish',
 'sports::fencing::karate',
 'sports::surfing::water skiing',
 'sports::mountain climbing::bouldering',
 'sports::soccer::rugby',
 'sports::basketball::volleyball',
 'sports::tennis::badminton',
 'sports::swimming::diving',
 'sports::boxing::mma',
 'sports::gymnastics::rhythmic gymnastics',
 'jobs::actor::film director',
 'jobs::firefighter::lifeguard',
 'jobs::professor::counselor',
 'jobs::doctor::pharmacist',
 'jobs::chef::baker',
 'jobs::astronaut::pilot',
 'nature::river::lake',
 'nature::mountain::valley',
 'nature::rainforest::jungle',
 'nature::volcano::earthquake',
 'nature::lightning::thunder',
 'nature::desert::oasis',
 'nature::moon::sun',
 'tech::printer::scanner',
 'tech::satellite::radar',
 'tech::iphone::android',
 'tech::playstation::xbox',
 'tech::robot::artificial intelligence',
 'tech::laptop::tablet',
 'tech::artificial intelligence::robot',
 'tech::headphones::speaker',
]);
let recentRandomTopicTypes=[];

function normalizeTopicWord(value){
 return String(value||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
}
function getTopicWords(topicItem){
 if(!topicItem) return null;
 if(Array.isArray(topicItem.en)&&topicItem.en.length>=2) return topicItem.en;
 if(Array.isArray(topicItem.ar)&&topicItem.ar.length>=2) return topicItem.ar;
 return null;
}
function getRandomTopicKey(topicItem){
 const words=getTopicWords(topicItem);
 if(!words) return '';
 return `${topicItem.catKey||'misc'}::${normalizeTopicWord(words[0])}::${normalizeTopicWord(words[1])}`;
}
function isSimilarRandomTopic(topicItem){
 return RANDOM_SIMILAR_TOPIC_KEYS.has(getRandomTopicKey(topicItem));
}
function rememberRandomTopicType(topicItem){
 if(genreMode!=='random') return;
 recentRandomTopicTypes.push(isSimilarRandomTopic(topicItem));
 if(recentRandomTopicTypes.length>4) recentRandomTopicTypes.shift();
}
function shouldUseSimilarRandomTopic(similarPool,regularPool){
 if(!similarPool.length) return false;
 if(!regularPool.length) return true;
 const lastWasSimilar=recentRandomTopicTypes[recentRandomTopicTypes.length-1]===true;
 const recentSimilarCount=recentRandomTopicTypes.filter(Boolean).length;
 let chance=RANDOM_SIMILAR_TOPIC_BASE_CHANCE;
 if(lastWasSimilar) chance=RANDOM_SIMILAR_TOPIC_COOLDOWN_CHANCE;
 else if(recentSimilarCount>=2) chance=0.08;
 return Math.random()<chance;
}
function pickTopic(pool){
 if(!pool||!pool.length) return TOPICS[0];
 let selectionPool=pool;
 if(genreMode==='random'&&pool.length>1){
  const similarPool=pool.filter(isSimilarRandomTopic);
  const regularPool=pool.filter(t=>!isSimilarRandomTopic(t));
  selectionPool=shouldUseSimilarRandomTopic(similarPool,regularPool)
   ? similarPool
   : (regularPool.length?regularPool:pool);
 }
 if(selectionPool.length===1){
  lastTopicIdx=TOPICS.indexOf(selectionPool[0]);
  rememberRandomTopicType(selectionPool[0]);
  return selectionPool[0];
 }
 let idx,attempts=0;
 do{ idx=Math.floor(Math.random()*selectionPool.length); attempts++; }
 while(attempts<20&&TOPICS.indexOf(selectionPool[idx])===lastTopicIdx);
 lastTopicIdx=TOPICS.indexOf(selectionPool[idx]);
 rememberRandomTopicType(selectionPool[idx]);
...
function startGame(){
 ...
 twistWordSwapped=false; twistSwappedList=[]; twistWordSwapApplied=false;
 insiderSpyIdx=-1;
 ...
 // Twist budget comes from the selected option, then gets locked by player count rules.
 const allowedTwistCounts = typeof getAllowedTwistCounts === 'function'
  ? getAllowedTwistCounts(players.length)
  : [1];
 if(!allowedTwistCounts.includes(selectedTwistCount)) selectedTwistCount = allowedTwistCounts[0] || 1;
 targetTwistsCount = twistsOn ? selectedTwistCount : 0;
 ...
 // Schedule twists — exact count from options, never clashing with GQ slots
 window.currentTwistSchedule = [];
 if(twistsOn && targetTwistsCount > 0){
  const canStartWithPreWord = targetTwistsCount > 1 && hasAvailablePreWordTwist();
  const midCount = targetTwistsCount - (canStartWithPreWord ? 1 : 0);
  ...
  if(canStartWithPreWord){
   try{
    if(firePreWordTwistSilent()){
     return;
    }
   } catch(err){
...
const MID_Q_TWISTS=['forced_vote','reverse_vote','time_pressure','whisper_round','third_letter_public','letter_count_public','two_clue_bluff'];

function hasAvailablePreWordTwist(){
...
let twistWordSwapApplied=false;
function isBaseOutsiderIdx(i){ return i===outsiderIdx||(outsiderCount===2&&i===outsiderIdx2); }
function getSwapPartnerIdx(i){
 if(!twistWordSwapped||!twistWordSwapApplied||twistSwappedList.length!==2) return -1;
 if(twistSwappedList[0]===i) return twistSwappedList[1];
 if(twistSwappedList[1]===i) return twistSwappedList[0];
 return -1;
}
function isOutsiderIdx(i){
 const partnerIdx=getSwapPartnerIdx(i);
 return partnerIdx>=0?isBaseOutsiderIdx(partnerIdx):isBaseOutsiderIdx(i);
}
function getCurrentOutsiderIndices(){
 const current=[];
 for(let i=0;i<players.length;i++){
  if(isOutsiderIdx(i)) current.push(i);
 }
 return current;
}
function getPrimaryOutsiderIdx(){
 const current=getCurrentOutsiderIndices();
 return current.length?current[0]:outsiderIdx;
}
function getOutsiderNames(){
 const current=getCurrentOutsiderIndices();
 if(!current.length) return ['?'];
 return current.map(i=>players[i]||'?');
}
function getOutsiderNamesText(){
 return getOutsiderNames().join(' & ');
}
...
let questionsAutoAdvancePending=false;

let twistRecentIds=[];
let twistRecentPlayers=[];
function resetTwistMemory(){
 twistRecentIds=[];
 twistRecentPlayers=[];
 lastFiredTwistId=null;
}
function pickTwistId(pool){
 if(!Array.isArray(pool)||!pool.length) return null;
 const recent=twistRecentIds.slice(-2);
 let candidates=pool.filter(id=>!recent.includes(id));
 if(!candidates.length) candidates=pool.slice();
 const chosen=candidates[Math.floor(Math.random()*candidates.length)];
 twistRecentIds.push(chosen);
 if(twistRecentIds.length>8) twistRecentIds.shift();
 lastFiredTwistId=chosen;
 return chosen;
}
function pickTwistPlayer(exclude=[]){
 const blocked=Array.isArray(exclude)?exclude:[exclude];
 let pool=players.map((_,i)=>i).filter(i=>!blocked.includes(i));
 if(!pool.length) return -1;
 if(pool.length>1){
  const recent=twistRecentPlayers.slice(-2);
  const filtered=pool.filter(i=>!recent.includes(i));
  if(filtered.length) pool=filtered;
 }
 const chosen=pool[Math.floor(Math.random()*pool.length)];
 twistRecentPlayers.push(chosen);
 if(twistRecentPlayers.length>8) twistRecentPlayers.shift();
 return chosen;
}
function getWordForPlayerIdx(i){
 return isOutsiderIdx(i)?(outsiderWord||'—'):(getGroupWord()||'—');
}
function getWordLetter(word,idx){
 const letters=Array.from(String(word||'').trim()).filter(Boolean);
 if(!letters.length) return '?';
 const safeIdx=Math.max(0,Math.min(idx,letters.length-1));
 return letters[safeIdx]||letters[letters.length-1]||'?';
}
...
function firePreWordTwist(){
 ...
 if(!avail.length){ activeTwist=null; showCover(); return false; }
 const id=pickTwistId(avail);
 if(!id){ activeTwist=null; showCover(); return false; }
 activeTwist={id};
 ...
 if(id==='word_swap'){
  ...
  showTwistScreen(
   '🔄',
   ar?'تبادل الكلمات':'Word Exchange',
   ar?`${players[a]} يقول كلمته لـ ${players[b]} و ${players[b]} يقول كلمته لـ ${players[a]} — وبعدها كل واحد يلعب بالكلمة الجديدة.`
     :`${players[a]} tells their word to ${players[b]}, and ${players[b]} does the same. After that, each player must play as the new word.`,
   `<div style="background:rgba(255,165,0,.1);border:2px solid var(--orange);border-radius:12px;padding:14px;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:var(--orange);">
    🔄 ${ar?'لما يشوف اللاعبان كلمتيهما، اللعبة تعطي كل واحد كلمة الثاني سراً':'Once both players see their words, the game will privately hand each one the other word'}
   </div>`,
   ar
  );
  consumeEvent();
 ...
 if(id==='insider_spy'){
  ...
  if(!insiders.length){ activeTwist=null; showCover(); return false; }
...
function firePreWordTwistSilent(){
 const ar=T.dir==='rtl';
 const avail=PRE_WORD_TWISTS.filter(id=>{
  if(gameMode==='story'||!topic) return false;
  if(id==='word_swap'&&players.length<3) return false;
  if(id==='insider_spy'&&players.length<3) return false;
  if(id==='private_letter'&&players.length<2) return false;
  return true;
 });
 if(!avail.length){ activeTwist=null; return false; }
 const id=pickTwistId(avail);
 if(!id){ activeTwist=null; return false; }
 activeTwist={id};
 ...
 if(id==='word_swap'){
  ...
  showTwistScreen(
   ...
   ar?`${players[a]} يقول كلمته لـ ${players[b]} و ${players[b]} يقول كلمته لـ ${players[a]} — وبعدها كل واحد يلعب بالكلمة الجديدة.`
     :`${players[a]} tells their word to ${players[b]}, and ${players[b]} does the same. After that, each player must play as the new word.`,
   `<div style="background:rgba(255,165,0,.1);border:2px solid var(--orange);border-radius:12px;padding:14px;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:var(--orange);">
    🔄 ${ar?'بعد ما يشوف اللاعبان كلمتيهما، اللعبة تبدل الدور الحقيقي بينهما سراً':'After both players see their words, the game secretly switches the real roles between them'}
   </div>`,
   ar
  );
  consumeEvent();
 ...
 if(id==='insider_spy'){
  const insiders=players.map((_,i)=>i).filter(i=>!isOutsiderIdx(i));
  if(!insiders.length){ activeTwist=null; return false; }
  const spyPool=insiders.length>1?insiders.filter(i=>i!==lastSpyIdx):insiders;
  insiderSpyIdx=spyPool[Math.floor(Math.random()*spyPool.length)];
  lastSpyIdx=insiderSpyIdx;
  showCover();
...
function fireRandomTwist(){
 ...
 let midAvail=[
  ...
  'third_letter_public','letter_count_public','two_clue_bluff',
  ...
  if((id==='third_letter_public'||id==='letter_count_public'||id==='two_clue_bluff')&&(gameMode==='story'||!topic)) return false;
 ...
 const withoutRecent=midAvail.filter(x=>!twistRecentIds.slice(-2).includes(x));
 if(withoutRecent.length>0) midAvail=withoutRecent;
 ...
 if(!midAvail.length){ renderSpotlightQ(); return; }
 const id=pickTwistId(midAvail);
 if(!id){ renderSpotlightQ(); return; }
 activeTwist={id};
 ...
 if(id==='forced_vote'){
  ...
  desc=ar
   ...
   :(T.reverseVoteFullPrompt?T.reverseVoteFullPrompt(players[reverseVotePlayerIdx]):`${players[reverseVotePlayerIdx]} — who do you trust is an Insider? Vote NOW — everyone sees your pick, and you cannot vote against this same player in the final vote!`);
  specificDesc=`<div style="background:rgba(59,143,255,.1);border:2px solid var(--blue);border-radius:12px;padding:14px;margin-top:12px;text-align:center;">
   ...
   <div style="font-size:12px;color:var(--blue);font-weight:700;margin-top:4px;">${T.everyoneSeesPick||(ar?'الكل يشوف اختياره — وما يقدر يصوّت ضد نفس الشخص بالنهاية!':'everyone sees their pick — and they cannot vote against the same player at the end!')}</div>
  </div>`;
 ...
 } else if(id==='third_letter_public'){
  emoji='🔤';
  title=ar?'الحرف الثالث بصوت عالٍ!':'Third Letter Public!';
  const pIdx=pickTwistPlayer([]);
  const word=getWordForPlayerIdx(pIdx);
  const letter=getWordLetter(word,2);
  desc=ar
   ?`${players[pIdx]} — قل الحرف الثالث من كلمتك قدام الكل!`
   :(T.thirdLetterPrompt?T.thirdLetterPrompt(players[pIdx]):`${players[pIdx]} — say the THIRD letter of your word out loud for everyone!`);
  specificDesc=`<div style="background:rgba(155,93,229,.1);border:2px solid var(--purple);border-radius:12px;padding:16px;margin-top:12px;text-align:center;">
   ...
   <div style="font-size:13px;color:var(--purple);margin-top:6px;">${ar?'يقول الحرف الثالث علنًا — والكل يسمع':'speaks the third letter out loud'}</div>
   <div style="font-size:42px;font-weight:900;color:var(--purple);margin-top:10px;letter-spacing:4px;">${letter}</div>
   <div style="font-size:11px;color:var(--muted);margin-top:6px;">${ar?'إذا كانت الكلمة أقصر من 3 حروف، استخدم آخر حرف ممكن':'If the word is shorter than 3 letters, use the last available letter'}</div>
  </div>`;

 } else if(id==='letter_count_public'){
  emoji='🔢';
  title=ar?'عدد الحروف بصوت عالٍ!':'Letter Count Public!';
  const pIdx=pickTwistPlayer([]);
  const word=getWordForPlayerIdx(pIdx);
  const count=Array.from(String(word||'').trim()).filter(Boolean).length;
  desc=ar
   ?`${players[pIdx]} — قل عدد حروف كلمتك بصوت واضح!`
   :(T.letterCountPrompt?T.letterCountPrompt(players[pIdx]):`${players[pIdx]} — say how many letters your word has out loud!`);
  specificDesc=`<div style="background:rgba(59,143,255,.1);border:2px solid var(--blue);border-radius:12px;padding:16px;margin-top:12px;text-align:center;">
   <div style="font-size:36px;margin-bottom:6px;">${AVATARS[pIdx%AVATARS.length]}</div>
   <div style="font-size:22px;font-weight:900;">${players[pIdx]}</div>
   <div style="font-size:13px;color:var(--blue);margin-top:6px;">${ar?'يعلن عدد الحروف الآن':'announces the letter count now'}</div>
   <div style="font-size:42px;font-weight:900;color:var(--blue);margin-top:10px;letter-spacing:4px;">${count}</div>
  </div>`;

 } else if(id==='two_clue_bluff'){
  emoji='🎭';
  title=ar?'خدعتين وواحدة كذبة!':'Two Clues, One Lie!';
  const pIdx=pickTwistPlayer([]);
  desc=ar
   ?`${players[pIdx]} — اعطِ الجماعة تلميحين عن كلمتك: واحد صدق وواحد كذب.`
   :(T.twoCluePrompt?T.twoCluePrompt(players[pIdx]):`${players[pIdx]} — give the group two clues about your word: one true and one fake.`);
  specificDesc=`<div style="background:rgba(255,165,0,.1);border:2px solid var(--orange);border-radius:12px;padding:16px;margin-top:12px;text-align:center;">
   <div style="font-size:36px;margin-bottom:6px;">${AVATARS[pIdx%AVATARS.length]}</div>
   <div style="font-size:22px;font-weight:900;">${players[pIdx]}</div>
   <div style="font-size:13px;color:var(--orange);margin-top:6px;">${ar?'تلميحان: واحد صحيح وواحد كذب':'two clues, one truth and one bluff'}</div>
   <div style="margin-top:12px;display:grid;gap:8px;">
    <div style="background:rgba(77,196,255,.12);border:1.5px solid rgba(77,196,255,.35);border-radius:12px;padding:10px 12px;font-weight:700;color:var(--blue);">${ar?'تلميح 1':'Clue 1'}</div>
    <div style="background:rgba(255,92,153,.12);border:1.5px solid rgba(255,92,153,.35);border-radius:12px;padding:10px 12px;font-weight:700;color:var(--pink);">${ar?'تلميح 2 (فيه كذبة)':'Clue 2 (one lie)'}</div>
   </div>
  </div>`;
  ...
  emoji='🏳️';
  title=T.playerSurrenders||(ar?'لاعب يستسلم!':'Player Surrenders!');
  ...
  // Two specific players swap words mid-game and the round logic follows the new holders
  emoji='🔄';
  title=ar?'تبادل الكلمات الآن!':'Word Exchange Now!';
  ...
  twistSwappedList=[a,b];
  twistWordSwapped=true;
  twistWordSwapApplied=true;
  desc=ar
   ?`${players[a]} يقول كلمته لـ ${players[b]} و ${players[b]} يقول كلمته لـ ${players[a]} — ومن الآن كل واحد يلعب بالكلمة الجديدة.`
   :`${players[a]} tells their word to ${players[b]}, and ${players[b]} does the same. From now on, each player must play as the new word.`;
  specificDesc=`<div style="background:rgba(255,165,0,.1);border:2px solid var(--orange);border-radius:12px;padding:14px;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:var(--orange);">
   🔄 ${T.newWordsApplyNow||(ar?'اللعبة تحسبهما حسب الكلمتين الجديدتين من الآن':'The game now treats them according to the new words')}
  </div>`;
  ...
  desc=ar
   ...
   :(T.firstLetterNowPrompt?T.firstLetterNowPrompt(players[pIdx]):`${players[pIdx]} — must say the FIRST LETTER of their word out loud right now!`);
  specificDesc=`<div style="background:rgba(155,93,229,.1);border:2px solid var(--purple);border-radius:12px;padding:16px;margin-top:12px;text-align:center;">
   <div style="font-size:36px;margin-bottom:6px;">${AVATARS[pIdx%AVATARS.length]}</div>
   <div style="font-size:22px;font-weight:900;">${players[pIdx]}</div>
  ...
  showTwistScreen(
   '🤫',
   T.scheduledSecretLetter||(ar?'حرف سري مجدول!':'Scheduled Secret Letter!'),
   ar?'لاعب واحد سيستلم حرفاً سرياً خلال شاشة كلمته الخاصة — بسرية تامة.'
...
function showEarlyVoteScreen(type){
 ...
 document.getElementById('twist-banner-el').innerHTML=`
  <div class="twist-banner" style="background:linear-gradient(135deg,${isInsiderVote?'#0d2247,#1a3a6a':'#7a3000,#b34a00'});">
   ...
   <div class="twist-label">${T.voteNowTitle||(ar?'صوّت الآن!':'Vote Now!')}</div>
   <div class="twist-text">${isInsiderVote?(T.votePromptTrusted?T.votePromptTrusted(voterName):(ar?`${voterName} — مين تثق فيه؟ (ما تقدر تصوّت ضده بالنهاية)`:`${voterName} — who do you trust? (You cannot vote against this player later)`)):(T.votePromptOutsider?T.votePromptOutsider(voterName):(ar?`${voterName} — مين الخارج؟`:`${voterName} — who is the Outsider?`))}</div>
  </div>`;
  ...
  <div class="vote-shell">
   <div class="vote-panel">
    <div class="vote-panel-head">
     <div>
      <div class="vote-panel-title">${isInsiderVote?(T.chooseTrustedTitle||(ar?'اختار الشخص الموثوق':'Choose your vote')):(T.chooseOutsiderTitle||(ar?'اختار الخارج':'Choose your vote'))}</div>
      <div class="vote-panel-sub">
       ${isInsiderVote?(T.pickTrustedSub||(ar?'اختر الشخص اللي تثق إنه داخل اللعبة (مو الخارج)':'Pick the person you trust is an Insider')):(T.pickOutsiderSub||(ar?'اختر الشخص اللي تشك إنه الخارج':'Pick the person you think is the Outsider'))}
      </div>
     </div>
     <div class="vote-panel-chip">${players.length-1} ${T.playersCountLbl||'players'}</div>
    </div>
    <div class="vote-select">${candidates}</div>
   </div>
   ${isInsiderVote?`<div class="vote-note vote-note-lock">🔒 ${T.trustReminderFinal||(ar?'تذكير: اللاعب اللي تثق فيه الآن، ما تقدر تصوّت ضده في التصويت النهائي':'Reminder: You cannot vote against this trusted player in the final vote.')}</div>`:''}
  </div>`;
...
function showPrivateLetterScreen(){
 ...
 document.getElementById('twist-banner-el').innerHTML=`
  <div class="twist-banner" style="background:linear-gradient(135deg,#1a3a6a,#0d2247);">
   ...
   <div class="twist-label">${T.secretInfoTitle||(ar?'حرف سري':'Secret Letter')}</div>
   <div class="twist-text">${T.secretInfoFor?T.secretInfoFor(players[receiver]):(ar?`${players[receiver]} — معلومة سرية!`:`${players[receiver]} — secret info!`)}</div>
  </div>`;
 document.getElementById('twist-details-el').innerHTML=`
  <div class="card" style="padding:18px;text-align:center;">
   <div style="font-size:13px;color:var(--muted);margin-bottom:12px;">${T.passPhoneLookAway?T.passPhoneLookAway(players[receiver]):(ar?'سلّم التلفون لـ '+players[receiver]+' — الكل يبعد عيونه!':'Pass phone to '+players[receiver]+' — everyone look away!')}</div>
   <div style="background:var(--s2);border-radius:12px;padding:16px;margin-bottom:12px;">
    <div style="font-size:13px;font-weight:700;color:var(--blue);margin-bottom:6px;">${T.firstLetterOf?T.firstLetterOf(players[target]):(ar?'الحرف الأول من كلمة '+players[target]+':':'First letter of '+players[target]+'s word:')}</div>
    <div style="font-size:52px;font-weight:900;color:var(--blue);letter-spacing:6px;">${letter}</div>
   </div>
   <div style="font-size:12px;color:var(--muted);">${T.tellNoOne||(ar?'لا تخبر أحد — استخدمها لصالحك!':'Tell no one — use this to your advantage!')}</div>
  </div>`;
 const okBtn=document.getElementById('btn-twist-ok');
 if(okBtn){ okBtn.style.display='flex'; okBtn.textContent=T.gotItContinue||(ar?'✅ حفظت — نكمل':'✅ Got it — Continue'); }
...
function showNextSwapCover(){
 ...
 document.getElementById('swap-sub').textContent=
  T.swapPassPhone?T.swapPassPhone(name,otherName):(ar?`سلّم التلفون لـ ${name} — الآن يستلم كلمة ${otherName} ويلعب بها 🔄 الباقين يبعدون عيونهم!`
    :`Pass phone to ${name} — they now receive ${otherName}'s word and must play with it 🔄 everyone else look away!`);
 document.getElementById('swap-warning').textContent=T.swapWarning||(ar?'⚠️ الكلمة اللي بتشوفها سرية!':'⚠️ The word you see is secret!');
 const showBtn=document.getElementById('btn-show-swap');
 if(showBtn) showBtn.textContent=T.receiveWord?T.receiveWord(otherName):(ar?`👁️ استلم كلمة ${otherName}`:`👁️ Receive ${otherName}'s word`);
 show('s-swap-reveal');
}
 ...
 if(inner) inner.innerHTML=`
  <div style="text-align:center;padding:0 8px;">
   <div style="font-size:13px;font-weight:700;color:var(--orange);margin-bottom:18px;background:rgba(255,165,0,.1);border:1.5px solid var(--orange);border-radius:30px;padding:7px 16px;display:inline-block;">
    🔄 ${T.wordSwapTitle||(ar?'تبادل الكلمات':'Word Swap')}
   </div>
   <div style="font-size:14px;color:var(--muted);margin-bottom:8px;">
    ${T.nowPlayingAs?T.nowPlayingAs(otherName):(ar?`هذه هي الكلمة التي ستلعب بها بدل ${otherName}:`:`This is the word you now play with instead of ${otherName}:`)}
   </div>
   <div style="font-size:15px;font-weight:700;color:var(--muted);margin-bottom:6px;">
    ${T.youPair?T.youPair(otherName):(ar?`${otherName} 🤝 أنت`:`${otherName} 🤝 You`)}
   </div>
   <div class="word-huge" style="margin:18px 0;">${receivedWord}</div>
   <div class="word-hint">${T.newWordHint||(ar?'هذه كلمتك الجديدة — من الآن تُعامل داخل اللعبة على أساسها':'This is your new word — from now on the game treats you as this word')}</div>
  </div>`;
...
function showWord(){
 ...
 if(gameMode==='story'){
  ...
  if(isOutsider&&outsiderMode==='knows'){
   ...
   const lbl=T.storyLbl||(T.dir==='rtl'?'القصة':'Story');
   wrap.innerHTML=`
 ...
 if(pendingPrivateLetter&&pendingPrivateLetter.receiver===currentIdx&&!pendingPrivateLetter.shown&&gameMode!=='story'){
  ...
  privatePanel.innerHTML=`
   <div style="background:linear-gradient(135deg,#1a3a6a,#0d2247);border:2px solid rgba(59,143,255,.55);border-radius:16px;padding:14px 12px;text-align:center;">
    ...
    <div style="font-size:13px;font-weight:700;color:rgba(255,255,255,.85);">${T.firstLetterOf?T.firstLetterOf(players[p.target]):(ar2?`حرف كلمة ${players[p.target]} الأول:`:`First letter of ${players[p.target]}'s word:`)}</div>
 ...
 if(insiderSpyIdx>=0&&currentIdx===insiderSpyIdx&&!isOutsider&&gameMode!=='story'){
  const ar2=T.dir==='rtl';
  const outsiderName=getOutsiderNamesText();
  ...
  spyPanel.innerHTML=`
   <div style="background:linear-gradient(135deg,#1a0533,#2d0b5e);border:2px solid rgba(155,93,229,.6);border-radius:18px;padding:18px 16px;text-align:${ar2?'right':'left'};">
    <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,.5);margin-bottom:12px;text-align:center;">🕵️ ${T.secretIntelTitle||(ar2?'معلومات سرية — لك وحدك':'Secret Intel — For Your Eyes Only')} 🕵️</div>
    <div style="display:flex;flex-direction:column;gap:10px;">
     <div style="background:rgba(255,255,255,.08);border-radius:12px;padding:12px 14px;display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:12px;font-weight:700;color:rgba(255,255,255,.55);">${T.secretGroupWord||(ar2?'كلمة الجماعة':'Group Word')}</span>
     ...
     <div style="background:rgba(255,59,92,.15);border:1.5px solid rgba(255,59,92,.4);border-radius:12px;padding:12px 14px;display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:12px;font-weight:700;color:rgba(255,180,180,.7);">${T.secretOutsiderWord||(ar2?'كلمة الخارج':'Outsider Word')}</span>
     ...
     <div style="background:rgba(255,149,0,.15);border:1.5px solid rgba(255,149,0,.4);border-radius:12px;padding:12px 14px;display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:12px;font-weight:700;color:rgba(255,200,120,.7);">${T.secretOutsiderIs||(ar2?'الخارج هو':'Outsider is')}</span>
      <span style="font-size:17px;font-weight:900;color:#ffbb55;">${outsiderName}</span>
     </div>
    </div>
    <div style="font-size:11px;color:rgba(255,255,255,.35);text-align:center;margin-top:12px;line-height:1.6;">${T.useWisely||(ar2?'تظاهر إنك ما تعرف — أو استخدم هالمعلومات بذكاء 😏':'Act like you know nothing — or use this wisely 😏')}</div>
   </div>`;
...
function nextPlayer(){
 playSound('tap'); haptic('light');
 currentIdx++;
 // word_swap fires only after both chosen players have seen their original words
 if(activeTwist&&activeTwist.id==='word_swap'&&!activeTwist.swapDone&&players.length>=3&&twistSwappedList.length===2){
  const bothSelectedPlayersSeen=twistSwappedList.every(i=>i<currentIdx);
  if(bothSelectedPlayersSeen){
...
function showQuestionsPhase(){
 ...
 questionsAutoAdvancePending=false;
 currentQIdx=0;
 gameQuestionActive=false;
 ...
 if(rem&&activeTwist){
  ...
  if(activeTwist.id==='time_pressure'){
   ...
   rem.innerHTML=`<div style="background:#fff9e6;border:2px solid #ffe680;border-radius:12px;padding:10px 14px;margin-bottom:10px;text-align:center;font-size:13px;font-weight:700;color:#9a6600;">⏱️ ${T.timePressureReminder||(ar?'كل إجابة لازم تكون خلال 10 ثواني!':'Each answer must be within 10 seconds!')}</div>`;
  } else if(activeTwist.id==='whisper_round'||activeTwist.id==='silent_round'){
   rem.style.display='block';
   rem.innerHTML=`<div style="background:#f0f4ff;border:2px solid #b3c6ff;border-radius:12px;padding:10px 14px;margin-bottom:10px;text-align:center;font-size:13px;font-weight:700;color:#1a3a9a;">🤫 ${T.whisperReminder||(ar?'كل الأجوبة لازم تكون بالهمس!':'All answers must be whispered!')}</div>`;
  } else if(activeTwist.id==='reverse_vote'){
   rem.style.display='block';
   rem.innerHTML=`<div style="background:#fff0f3;border:2px solid #ffc5d0;border-radius:12px;padding:10px 14px;margin-bottom:10px;text-align:center;font-size:13px;font-weight:700;color:var(--red);">🔃 ${T.reverseVoteReminder||(ar?'تذكير: التصويت العكسي — الأكثر أصوات يفوز!':'Reminder: Reverse Vote — most voted wins!')}</div>`;
  } else {
...
function fireGameQuestion(){
 ...
 const qs=(window.getGameQuestionsForLang&&window.getGameQuestionsForLang(lang))||(ar?GAME_QUESTIONS_AR:GAME_QUESTIONS_EN);
 ...
 document.getElementById('twist-banner-el').innerHTML=`
  <div class="twist-banner" style="background:linear-gradient(135deg,#1a3a6a,#0d2247);">
   ...
   <div class="twist-text">${T.gqSpecificTitle||(ar?'سؤال للاعب محدد!':'A question for a specific player!')}</div>
  </div>`;
 document.getElementById('twist-details-el').innerHTML=`
  <div class="card" style="padding:20px;text-align:center;">
   ...
   <div style="font-size:13px;color:var(--muted);margin-bottom:12px;">${T.gqSpecificPrompt||(ar?'أجب بصدق — الكل يستمع ويحكم:':'Answer honestly — everyone listens:')}</div>
   ...
   <div style="font-size:12px;color:var(--muted);margin-top:10px;line-height:1.6;">${T.gqOutsiderHint||(ar?'الخارج يعرف كلمة مختلفة — جوابه سيكشفه':'The Outsider has a different word — their answer will expose them')}</div>
  </div>`;
 const _gqOkBtn=document.getElementById('btn-twist-ok');
 if(_gqOkBtn){ _gqOkBtn.textContent=T.gqAnsweredBtn||(ar?'✅ أجبنا — نكمل':'✅ Answered — Continue'); _gqOkBtn.style.display='flex'; }
...
let _timePressureKey=null;

function stopTimePressureTimer(){
 if(_timePressureInterval){
  clearInterval(_timePressureInterval);
  _timePressureInterval=null;
 }
 _timerGeneration++;
 _timePressureKey=null;
}

function syncTimePressureDisplay(secs,resetArc){
 const num=document.getElementById('timer-num');
 const arc=document.getElementById('timer-arc');
 const safeSecs=Math.max(0,Math.min(10,secs));
 const circ=2*Math.PI*28;
 if(num){
  num.textContent=safeSecs;
  num.classList.toggle('timer-urgent',safeSecs<=3);
 }
 if(arc){
  const progress=resetArc?0:(10-safeSecs)/10;
...
function renderSpotlightQ(){
 ...
 if(currentQIdx>=questions.length){
  ...
  if(!questionsAutoAdvancePending){
   questionsAutoAdvancePending=true;
   setTimeout(()=>{
    questionsAutoAdvancePending=false;
    if(currentQIdx>=questions.length){
     startVotePhase();
 ...
 if(isTimePressure&&!_timerPaused){
  const timerKey=`${currentMatchRound}:${currentQIdx}`;
  if(_timePressureKey!==timerKey){
   stopTimePressureTimer();
   _timerSecsLeft=10;
   startTimerCountdown(timerKey);
  } else {
   syncTimePressureDisplay(_timerSecsLeft,false);
  }
 } else if(_timePressureInterval){
  stopTimePressureTimer();
 }
}

function startTimerCountdown(timerKey){
 // Snapshot generation so stale closures can self-abort
 _timePressureKey=timerKey||`${currentMatchRound}:${currentQIdx}`;
 const myGen=_timerGeneration;
 let secs=Math.max(0,Math.min(10,Number(_timerSecsLeft)||10));
 syncTimePressureDisplay(secs,true);
 _timePressureInterval=setInterval(()=>{
  ...
  syncTimePressureDisplay(secs,false);
  if(secs<=0){
   stopTimePressureTimer();
...
function startVotePhase(){
 ...
 if(reverseVotePlayerIdx>=0&&reverseVoteResult>=0){
  ...
  reminders.push(`<div style="background:#e8f4fd;border:2px solid #7ec8e3;border-radius:12px;padding:10px 14px;margin-bottom:12px;text-align:center;font-size:13px;font-weight:700;color:#1565c0;">🔃 ${T.trustedFinalSummary?T.trustedFinalSummary(trusterName,trustedName):(ar?`${trusterName} اختار ${trustedName} كشخص موثوق — ما يقدر يصوّت ضده بالنهاية`:`${trusterName} trusted ${trustedName} — cannot vote against them in final voting`)}</div>`);
 }
...
function renderOpenVote(){
 ...
 const voteStepLabel=ar?'الدور':'Turn';
 ...
 phasesDiv.innerHTML=`
  <div class="vote-shell">
   <div class="vote-hero">
    <div class="vote-hero-main">
     <div class="vote-hero-avatar">${av}</div>
     <div class="vote-hero-copy">
      <div class="vote-hero-name">${voterName}</div>
      <div class="vote-hero-sub">${T.voteWho}</div>
     </div>
    </div>
    <div class="vote-step">${voteStepLabel} ${currentOpenVoter+1}/${openVoteOrder.length}</div>
   </div>
   <div class="vote-notes">
    ${isForced?`<div class="vote-note vote-note-warn">⚠️ ${T.mustVoteForcedLabel||'Must vote for'} ${players[twistForcedTargetIdx]}</div>`:''}
    ${hasTrustLock?`<div class="vote-note vote-note-lock">🔒 ${T.cannotVoteAgainst?T.cannotVoteAgainst(players[trustLockedTarget]):(ar?`لا يمكن التصويت ضد ${players[trustLockedTarget]}`:`Cannot vote against ${players[trustLockedTarget]}`)}</div>`:''}
   </div>
   <div class="vote-panel">
    <div class="vote-panel-head">
     <div>
      <div class="vote-panel-title">${T.chooseVote||(ar?'اختار التصويت':'Choose a vote')}</div>
      <div class="vote-panel-sub">${T.tapPlayerVoteFor||(ar?'اضغط على اسم اللاعب اللي تبغى تصوّت له':'Tap the player you want to vote for')}</div>
     </div>
     <div class="vote-panel-chip">${players.length-1} ${T.playersCountLbl||'players'}</div>
    </div>
...
function showTally(){
 ...
 const leadName=players[leadIdx]||'—';
 const leadVotes=counts[leadIdx]||0;
 document.getElementById('tally-list').innerHTML=`
  <div class="tally-summary">
   <div class="tally-summary-main">
    <div class="tally-summary-label">${T.tallyTitle||'Vote Results'}</div>
    <div class="tally-summary-name">${AVATARS[leadIdx%AVATARS.length]} ${leadName}</div>
   </div>
   <div class="tally-summary-pill">${T.voteCount(leadVotes)}</div>
  </div>
  ${sorted.map(i=>{
...
function showReveal(){
 ...
 const currentOutsiders=getCurrentOutsiderIndices();
 const outsiderNamesText=getOutsiderNamesText();
 const outsiderVoteTotal=currentOutsiders.reduce((sum,i)=>sum+(counts[i]||0),0);

 // Reverse vote twist flips who wins
 let groupWon=currentOutsiders.some(i=>topVoted.includes(i));
 ...
 // Outsider name + words
 document.getElementById('reveal-name').textContent=outsiderNamesText;
 ...
 document.getElementById('verdict-wrap').innerHTML=groupWon
  ?`<div class="result-verdict verdict-win">🎉 ${T.groupWon(outsiderNamesText,outsiderVoteTotal)}</div>`
 ...
 if(earlyWrap){
  ...
  if(reverseVoteResult>=0&&reverseVotePlayerIdx>=0){
   ...
   earlyHtml+=`<div class="micro-reward ${correct?'mr-mvp':'mr-close'}" style="width:100%;justify-content:center;margin-bottom:4px;">
    🔃 ${players[reverseVotePlayerIdx]} ${T.trustedVerb||(ar2?'وثق في':'trusted')} ${players[reverseVoteResult]} ${correct?(T.correctShort||(ar2?'✅ صح':'✅ correct')):(T.wrongShort||(ar2?'❌ غلط':'❌ wrong'))}
   </div>`;
  }
 ...
 // Buttons
 const isLastRound=currentMatchRound>=matchRoundCount;
...
function renderPunishment(groupWon, counts){
 ...
 const loserIdx=getPrimaryOutsiderIdx();
 ...
 const punishLabel={'truth':ar?'صراحة':'Truth','dare':ar?'جرأة':'Dare','guess':ar?'فرصة ثانية':'Second Chance'};
 
 wrap.innerHTML=`
  <div class="punish-wrap" style="text-align:center;gap:14px;">
   <div class="punish-title" style="margin-bottom:4px;">${T.punishHeader||'⚡ Punishment'} — ${loserName}</div>
   <div style="font-size:13px;color:var(--muted);margin-bottom:4px;">${T.wheelChoosesFinalChallenge||(T.dir==='rtl'?'العجلة تختار التحدي النهائي':'The wheel chooses the final challenge')}</div>
   <div id="punish-wheel-wrap" style="display:flex;flex-direction:column;align-items:center;gap:14px;">
    <div id="punish-wheel" style="width:132px;height:132px;border-radius:50%;border:4px solid var(--pink);display:flex;align-items:center;justify-content:center;font-size:54px;background:linear-gradient(135deg,var(--surface2),var(--surface3));box-shadow:var(--glow-pink);transition:transform 0.1s;">🎰</div>
    <button onclick='spinPunishWheel(${JSON.stringify(punishments)},${JSON.stringify(punishEmoji)},${JSON.stringify(punishLabel)},${canGuess})' style="background:linear-gradient(135deg,var(--pink),var(--orange));color:#fff;border:none;border-radius:30px;padding:14px 32px;font-size:16px;font-weight:900;cursor:pointer;font-family:'Cairo',sans-serif;box-shadow:var(--glow-pink);" id="spin-btn">
     ${T.spinWheel||(T.dir==='rtl'?'🎯 دوّر العجلة!':'🎯 Spin the Wheel!')}
    </button>
   </div>
 ...
 function step(){
  if(tick>=totalTicks){
   ...
   setTimeout(()=>{
    ...
    if(!area) return;
    const chosenLabel=labels[chosen]||chosen;
    const labelPill=`<div style="display:inline-flex;align-items:center;gap:8px;padding:7px 14px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);font-size:13px;font-weight:800;color:var(--text);margin-bottom:12px;">${emojis[chosen]} ${chosenLabel}</div>`;
    if(chosen==='guess'&&canGuess){area.innerHTML=`<div class="punish-result" style="background:linear-gradient(135deg,rgba(61,220,90,.12),rgba(61,220,90,.06));border-color:rgba(61,220,90,.35);color:var(--green);">${labelPill}<div>${T.secondChanceLabel||(T.dir==='rtl'?'فرصة ثانية!':'Second chance!')}</div></div>`;showGuessChallenge(area);}
    else if(chosen==='truth'){
     area.innerHTML=`<div class="punish-result" style="background:#e8f4fd;border-color:#7ec8e3;color:#1565c0;">${labelPill}<div>💬 ${T.punishTruthResult||'Ask them any question — they must answer honestly!'}</div></div>`;
     revealWords();
    } else {
     area.innerHTML=`<div class="punish-result" style="background:#fff0f3;border-color:#ffc5d0;color:var(--red);">${labelPill}<div>🎭 ${T.punishDareResult||'Assign a dare — they must do it!'}</div></div>`;
     revealWords();
    }
   },600);
  } else {
...
function revealWords(){
 ...
 if(gMode==='story'&&sIdx>=0){
  ...
  if(realEl) realEl.textContent=story.e+' '+(T.groupStoryCardLbl||(T.dir==='rtl'?'قصة الجماعة':'Group\'s story'));
  if(fakeEl) fakeEl.textContent=story.e+' '+(T.outsiderStoryCardLbl||(T.dir==='rtl'?'قصة الخارج':'Outsider\'s story'));
...
function findMostVotedNonOutsider(counts){
 ...
 const fallbackOutsider=getPrimaryOutsiderIdx();
 return best>=0?best:(fallbackOutsider===0?1:0);
}
...
function submitGuess(encW,encCorrect,idx){
 ...
 if(isRight){
  ...
  const loserName=getOutsiderNamesText();
...
function getMatchSummary(sourceHistory=matchRoundHistory, sourceScores=matchScores){
 ...
 const groupWins=history.filter(r=>r.groupWon).length;
 const outsiderWins=Math.max(0,roundsPlayed-groupWins);
 const winnerPts=winner[1]||0;
 const runnerUpPts=runnerUp[1]||0;
 return {
  roundsPlayed,
  totalRounds:matchRoundCount,
  groupWins,
  outsiderWins,
  winnerName:winner[0]||'?',
  winnerPts,
  ...
  runnerUpPts,
  leadMargin:runnerUp?Math.max(0,winnerPts-runnerUpPts):winnerPts,
  entries,
 };
}

function renderMatchStats(summary,mode='full'){
 const items=mode==='compact'
  ?[
    {label:T.roundsPlayedLbl||'Rounds played',value:`${summary.roundsPlayed}/${summary.totalRounds||matchRoundCount}`},
    {label:T.groupWinsLbl||'Group wins',value:summary.groupWins},
    {label:T.outsiderWinsLbl||'Outsider wins',value:summary.outsiderWins},
   ]
  :[
    {label:T.matchFormatLbl||'Match format',value:`${summary.roundsPlayed}/${summary.totalRounds||matchRoundCount}`},
    {label:T.groupWinsLbl||'Group wins',value:summary.groupWins},
    {label:T.outsiderWinsLbl||'Outsider wins',value:summary.outsiderWins},
    {label:T.leadLbl||'Lead',value:`+${summary.leadMargin}`},
   ];
 return `<div class="match-stats ${mode==='compact'?'compact':''}">
  ${items.map(item=>`
...
function renderScoreboard(){
 ...
 const medals=['🥇','🥈','🥉'];
 const rows=summary.entries.map(([name,pts],i)=>{
 ...
 return `<div class="scoreboard">
  <div class="sb-title">🏆 Scoreboard — ${T.roundLbl(Math.min(currentMatchRound,matchRoundCount),matchRoundCount)}</div>
  ${renderMatchStats(summary,'compact')}
  ${rows}
 </div>`;
}
...
function nextRound(){
 ...
 stopTimePressureTimer(); _timerPaused=false; _timerSecsLeft=10;
...
function showMatchOver(){
 ...
 const winnerPts=summary.winnerPts;
 // Winner card
 document.getElementById('match-winner-wrap').innerHTML=`
  <div class="match-winner-card" style="padding:22px 18px;">
   <span class="match-crown">👑</span>
   <div class="mw-label">${T.matchWinnerLbl||'بطل اللعبة'}</div>
   <div class="mw-name" style="text-align:center;">${winnerName}</div>
   <div class="mw-pts">${winnerPts} ${T.ptsLbl||'pts'}</div>
   <div style="margin-top:12px;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted);">${T.matchStatsLbl||'Match stats'}</div>
   ${renderMatchStats(summary)}
  </div>`;
 ...
 showMotivator(`👑 ${winnerName} ${T.dir==='rtl'?'فاز!':'wins!'}`);
 show('s-match-over');
}
...
function startNewMatch(){
    ...
    earlyVotePlayerIdx = -1; earlyVoteResult = -1; reverseVotePlayerIdx = -1; reverseVoteResult = -1;
    lockedEarlyVotes = {}; trustVoteLocks = {};
    insiderSpyIdx = -1; twistWordSwapped = false; twistSwappedList = []; twistWordSwapApplied = false;
    pendingPrivateLetter = null;
...
function saveHistory(){
 ...
 try{
  ...
  h.unshift({
   ...
   pts:summary.winnerPts,
   matchRoundCount,
   roundsPlayed:summary.roundsPlayed,
   summary:{
    groupWins:summary.groupWins,
    outsiderWins:summary.outsiderWins,
    leadMargin:summary.leadMargin,
   },
   scores:{...matchScores},
   rounds:matchRoundHistory.map(r=>({r:r.round,outsider:r.outsider,groupWon:r.groupWon,winner:r.roundWinner,scores:{...r.scores}})),
...
function renderRoundScoreChips(scores){
 const entries=Object.entries(scores||{}).filter(([,pts])=>pts>0).sort((a,b)=>b[1]-a[1]);
 if(!entries.length) return '';
 return `<div class="history-score-chips">
  ${entries.map(([name,pts],i)=>`<span class="history-score-chip${i===0?' top':''}">${name} <strong>+${pts}</strong></span>`).join('')}
 </div>`;
}
...
function renderHistory(){
 ...
 if(!h.length){
  listEl.innerHTML=`<div style="text-align:center;padding:40px 0;color:var(--muted);">
   ...
   <div style="font-size:13px;margin-top:8px;color:var(--muted);">${T.historyEmptyHint|| (T.dir==='rtl'?'العبوا مباراة كاملة حتى يظهر السجل هنا.':'Finish a full match to see it here.')}</div>
  </div>`;
  return;
 }
 listEl.innerHTML=h.map((g,idx)=>{
  const rounds=g.rounds||[];
  const roundsPlayed=g.roundsPlayed ?? rounds.length;
  const totalRounds=g.matchRoundCount ?? g.summary?.totalRounds ?? rounds.length;
  const groupWins=g.summary?.groupWins ?? rounds.filter(r=>r.groupWon).length;
  const outsiderWins=g.summary?.outsiderWins ?? Math.max(0,roundsPlayed-groupWins);
  const leadMargin=g.summary?.leadMargin ?? 0;
  const players=(g.players||[]);
  const roundDetails=rounds.map(r=>`
   <div class="history-round">
    <div class="history-round-top">
     <span class="history-round-name">${T.roundLblShort||'R'} ${r.r}</span>
     <span class="history-round-winner">🥇 ${r.winner||'—'}</span>
    </div>
    <div class="history-round-detail">
     ${T.outsiderLbl||'Outsider'}: <strong>${r.outsider}</strong> · ${r.groupWon?(T.caughtShort||'caught'):(T.escapedShort||'escaped')}
    </div>
    ${renderRoundScoreChips(r.scores)}
   </div>`).join('');
  const titleLine=(T.historyTapHint|| (T.dir==='rtl'?'اضغط على أي مباراة للتفاصيل':'Tap a game for details'));
  return `<button type="button" class="history-card history-card-btn" onclick="toggleHistoryCard(this)" aria-expanded="false">
   <div class="hc-top">
    <span class="hc-date">${g.date}</span>
    <span class="hc-winner">👑 ${g.winner} (${g.pts} ${T.ptsLbl||'pts'})</span>
   </div>
   <div class="hc-subline"><span>${titleLine}</span><span class="hc-cue-arrow">⌄</span></div>
   <div class="hc-player-chips">${players.map(p=>`<span class="hc-player-chip">${p}</span>`).join('')}</div>
   <div style="margin-bottom:10px;">${renderMatchStats({roundsPlayed,totalRounds,groupWins,outsiderWins,leadMargin},'compact')}</div>
   <div class="history-details">
    <div class="history-section-label">${T.roundLblShort||'Round'} ${T.dir==='rtl'?'تفاصيل':'details'}</div>
    <div class="history-rounds">${roundDetails || `<div class="history-empty-rounds">${T.historyEmptyHint||''}</div>`}</div>
   </div>
  </button>`;
 }).join('');
}
